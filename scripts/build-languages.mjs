/**
 * Generates src/data/languages.ts.
 *
 ...
 *
 * `dev`, `check`, `build` and `preview` each run this first. A list still
 * inside MAX_AGE_HOURS is left alone, so only the first run of the session
 * reaches the network; `--force` fetches regardless. src/data/languages.ts is
 * gitignored, so a fresh clone has nothing to keep and must reach WakaTime.
 */
...
/** How long a generated list is taken as current. `--force` ignores it. */
 ...
 * on this disk, which a checkout or a copy resets, while the header records
 * when the data was actually fetched.
 */
function ageOfList() {
  if (!existsSync(OUT)) return null;
  const stamp = readFileSync(OUT, 'utf8').match(/^\/\/ Generated: (\S+)$/m)?.[1];
...
// seconds matter: ageOfList() parses this back
const generatedAt = new Date().toISOString().replace(/\.\d+Z$/, 'Z');

writeFileSync(
  OUT,
  `// GENERATED by scripts/build-languages.mjs — do not edit by hand.
// Source: ${SHARE_URL}
...
export const languages: Language[] = ${JSON.stringify(data, null, 2)};
`
);
